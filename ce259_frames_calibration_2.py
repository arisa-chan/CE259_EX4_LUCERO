...
# ============================================================
# CONCRETE SLABS – ShellMITC4 elements
# ============================================================
E_slab  = 4700e3 * math.sqrt(19.4)   # elastic modulus, kPa (same concrete)
nu_slab = 0.2
rho_slab = 0.0                         # mass density (static analysis)

# Shell sections
ops.section('ElasticMembranePlateSection',  9, E_slab, nu_slab, 0.25, rho_slab)  # 1st floor: t=250mm
ops.section('ElasticMembranePlateSection', 10, E_slab, nu_slab, 0.20, rho_slab)  # 2nd floor: t=200mm

# Interior nodes (4 per panel) – not present on any beam line
# z = 4.3 m – panel 7-8-10-9
ops.node(47,       4.7/3,          2.85/3, 4.3)
ops.node(48,   2*4.7/3,            2.85/3, 4.3)
ops.node(49,       4.7/3,        2*2.85/3, 4.3)
ops.node(50,   2*4.7/3,          2*2.85/3, 4.3)
# z = 4.3 m – panel 9-10-12-11
ops.node(51,       4.7/3,   2.85 +   2.85/3, 4.3)
ops.node(52,   2*4.7/3,     2.85 +   2.85/3, 4.3)
ops.node(53,       4.7/3,   2.85 + 2*2.85/3, 4.3)
ops.node(54,   2*4.7/3,     2.85 + 2*2.85/3, 4.3)
# z = 7.6 m – panel 13-14-16-15
ops.node(55,       4.7/3,          2.85/3, 7.6)
ops.node(56,   2*4.7/3,            2.85/3, 7.6)
ops.node(57,       4.7/3,        2*2.85/3, 7.6)
ops.node(58,   2*4.7/3,          2*2.85/3, 7.6)
# z = 7.6 m – panel 15-16-18-17
ops.node(59,       4.7/3,   2.85 +   2.85/3, 7.6)
ops.node(60,   2*4.7/3,     2.85 +   2.85/3, 7.6)
ops.node(61,       4.7/3,   2.85 + 2*2.85/3, 7.6)
ops.node(62,   2*4.7/3,     2.85 + 2*2.85/3, 7.6)

# ShellMITC4 elements – nodes ordered counterclockwise viewed from +z
# (i-end, j-end, k-end, l-end) = (SW, SE, NE, NW) in plan

# --- z = 4.3 m, panel 7-8-10-9 (section 9, t=250mm) ---
for _conn in [
    ( 7, 19, 47, 25), (19, 20, 48, 47), (20,  8, 29, 48),
    (25, 47, 49, 26), (47, 48, 50, 49), (48, 29, 30, 50),
    (26, 49, 21,  9), (49, 50, 22, 21), (50, 30, 10, 22),
]:
    ops.element('ShellMITC4', _ele_tag, *_conn, 9)
    _ele_tag += 1

# --- z = 4.3 m, panel 9-10-12-11 (section 9, t=250mm) ---
for _conn in [
    ( 9, 21, 51, 27), (21, 22, 52, 51), (22, 10, 31, 52),
    (27, 51, 53, 28), (51, 52, 54, 53), (52, 31, 32, 54),
    (28, 53, 23, 11), (53, 54, 24, 23), (54, 32, 12, 24),
]:
    ops.element('ShellMITC4', _ele_tag, *_conn, 9)
    _ele_tag += 1

# --- z = 7.6 m, panel 13-14-16-15 (section 10, t=200mm) ---
for _conn in [
    (13, 33, 55, 39), (33, 34, 56, 55), (34, 14, 43, 56),
    (39, 55, 57, 40), (55, 56, 58, 57), (56, 43, 44, 58),
    (40, 57, 35, 15), (57, 58, 36, 35), (58, 44, 16, 36),
]:
    ops.element('ShellMITC4', _ele_tag, *_conn, 10)
    _ele_tag += 1

# --- z = 7.6 m, panel 15-16-18-17 (section 10, t=200mm) ---
for _conn in [
    (15, 35, 59, 41), (35, 36, 60, 59), (36, 16, 45, 60),
    (41, 59, 61, 42), (59, 60, 62, 61), (60, 45, 46, 62),
    (42, 61, 37, 17), (61, 62, 38, 37), (62, 46, 18, 38),
]:
    ops.element('ShellMITC4', _ele_tag, *_conn, 10)
    _ele_tag += 1
...
# Concrete density approx 24 kN/m^3.
# Beams (0.3x0.5m) -> Area = 0.15 m^2 -> w = 3.6 kN/m
# Columns (0.3x0.3m) -> Area = 0.09 m^2 -> w = 2.16 kN/m
# Y-axis beams (ele 1-24): local y is +Z, so downwards gravity (-Z) is Wy = -3.6
ops.eleLoad('-ele', *list(range(1, 25)), '-type', '-beamUniform', -3.6, 0.0, 0.0)
# X-axis beams (ele 25-42): local y is -Z, so downwards gravity (-Z) is Wy = +3.6
ops.eleLoad('-ele', *list(range(25, 43)), '-type', '-beamUniform', 3.6, 0.0, 0.0)
# Columns (ele 43-54): local x is +Z, so downwards gravity (-Z) is Wx = -2.16
